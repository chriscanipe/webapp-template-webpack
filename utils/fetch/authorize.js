...
if (process.env.CLIENT_SECRETS_FILE) {
  ...
  CLIENT_SECRETS_FILE = path.join(
    os.homedir(),
    ".axios_kit_google_client_secrets.json"
  );
}
...
try {
  ...
  if (e.code === "ENOENT") {
    console.log(
      chalk.red(
        "Could not find the client secrets file at %s. Are you sure it's there? Talk to Alex or Matt or Gerald if you don't have it"
      ),
      CLIENT_SECRETS_FILE
    );
  } else {
...
const GOOGLE_TOKEN_FILE = process.GOOGLE_TOKEN_FILE
